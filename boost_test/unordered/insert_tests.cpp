...
void unique_insert_tests1(X*, test::random_generator generator)
{
            ...
            it != v.end(); ++it)
    {
		...
		BOOST_TEST(r1.first->first == r2.first->first);
		BOOST_TEST(r1.first->second == r2.first->second);
...
void insert_tests2(X*, test::random_generator generator)
{
...

    {
            ...
            it = v.begin(); it != v.end(); ++it)
        {
            ...
            pos = x.insert(pos, *it);
            tracker_iterator r2 = tracker.insert(tracker.begin(), *it);
			BOOST_TEST(pos->first == r2->first);
			BOOST_TEST(pos->second == r2->second);
			tracker.compare_key(x, *it);
...
{
	size_t operator()(const insert_tests::overloaded_constructor & oc) const
	{
		return hash_value(oc);
	}
};
}
namespace insert_tests
